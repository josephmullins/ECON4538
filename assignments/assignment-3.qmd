...
and $\alpha_{i}$, $\epsilon_{it}$, and $\eta_{it}$ are iid, with $\epsilon_{it}\sim N(0,\sigma_{\epsilon}^2)$, $\eta_{it}\sim N(0,\sigma^2_{\eta})$. Let $\sigma^2_{\alpha} = \mathbb{V}[y_{it}]$ and let $\mathbb{E}[\alpha_{i}]=0$ such that $\mathbb{E}[y_{it}] = \mu_{t}$.
...
3. Following the steps in Recitation 4, use your answers to question 2 to construct estimates of the parameters of the income process, $(\mu,\rho,\sigma^2_{\epsilon},\sigma^2_{\eta},\sigma^2_{\alpha})$. **Note**: you will need to adjust the strategy above to account for the fact that the PSID only has data every two years. How does the estimate of $\rho$ compare to the estimate
